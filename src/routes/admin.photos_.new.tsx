...
import { useGovernorates, useHeritageCategories, useTags, useAdminAddPhoto } from "@/lib/queries";
import { toast } from "sonner";

export const Route = createFileRoute("/admin/photos_/new")({
  component: NewPhoto,
});
...
function NewPhoto() {
  ...
  const addMutation = useAdminAddPhoto();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (files.length === 0) return toast.error("الرجاء اختيار صورة");
  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <h1 className="text-3xl font-bold">إضافة صورة</h1>
      <p className="text-muted-foreground mt-1 text-sm">
        يمكن رفع عدة صور للمكان نفسه. الصورة الأولى ستكون صورة الغلاف.
      </p>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-8">
        ...
        >
          ...
          {files.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
              {files.map((f, i) => (
                <div key={i} className="relative group border rounded overflow-hidden aspect-video">
                  <img src={URL.createObjectURL(f)} className="w-full h-full object-cover" />
                  <button type="button" onClick={(e) => { e.stopPropagation(); setFiles(files.filter((_, idx) => idx !== i)); }} className="absolute top-1 right-1 bg-black/50 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <X className="size-4" />
                  </button>
            ...
            <>
              ...
              <p className="font-semibold">اسحب الصورة هنا أو انقر للاختيار</p>
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-foreground/5">
          <button type="submit" disabled={addMutation.isPending} className="flex items-center justify-center gap-2 text-sm bg-accent text-accent-foreground px-5 py-2.5 rounded-sm hover:opacity-90 disabled:opacity-50">
